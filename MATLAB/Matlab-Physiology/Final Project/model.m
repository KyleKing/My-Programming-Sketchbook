...
% See also trial1.m
  ...
  % oscillations may become prominent feature of the dynamics, as shown by Tolic Ì? et al. [40]. Here we choose as standard reference
  ...
  G1 = linspace(G,G,length(psi_in));
   ...
   % t = 1;
    ...
    psi_id = ((G1(t-1)/Vg)/gamma_id)*(psi0+(psiid-psi0)/(1+exp(-alpha_id*log((Ii/Iid)*((1/Vi)+((phi*tau_i)^(-1))))))); % A.2
    ...
    mu = mu_0*(1+(Ii/Vi)/Iai)*(1+exp(alpha_1-(G1(t-1)/Vg)/gamma_1-alpha_2*(1-(G1(t-1)/Vg)/gamma_2)^4))^(-1); % A.7
    psi_ng = psing*(1+exp(alpha_ngg*(1-(G1(t-1)/Vg)/gamma_ng)))^(-1)*(1+exp(alpha_ngi*(1-(Ii/Vg)/Ing)))^(-1); %  Eq. 20

    % Differential Equations
    dGdt = psi_in - psi_ii - psi_id + psi_gr - psi_gx % Eq. 13
    % S=dsolve(diff(G) == dGdt)
    G1(t) = G1(t-1)+dGdt(t);
    % dGdt(t) = psi_in(t) - psi_ii - psi_id + psi_gr - psi_gx; % Eq. 13
