on run {arg}
	if (arg is "on") then
		set DesiredStatus to true
	else
		set DesiredStatus to false
	end if

	tell application "System Preferences"
		activate
		set the current pane to pane "com.apple.preferences.sharing"
	end tell
		...
		tell process "System Preferences"
			tell window "Sharing"
				...
				tell theCheckbox
					...
					if checkboxStatus is not DesiredStatus then click theCheckbox
				end tell
				click
				delay 0.25
